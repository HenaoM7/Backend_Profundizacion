...
import { getInstitucionConfig } from '../models/institucionModel.js';
import { randomUUID }     from 'crypto';

const BASE_URL = process.env.BASE_URL ?? 'https://plataformaiush.edu.co';

const buildCertificateHtml = (logo, colors) => `<!DOCTYPE html>
<html lang="es">
<head>
<meta charset="UTF-8" />
...
const renderTokens = (html, { nombre, curso, fecha, firma1, firma2 }) =>
  html
    .replace(/\{\{nombre\}\}/g,  nombre  ?? '')
    .replace(/\{\{curso\}\}/g,   curso   ?? '')
    .replace(/\{\{fecha\}\}/g,   fecha)
    .replace(/\{\{firma1\}\}/g,  firma1  ?? '')
    .replace(/\{\{firma2\}\}/g,  firma2  ?? '');

const _buildCertificado = async (userId, courseId) => {
  const [datos, config] = await Promise.all([
    certRepo.findDatosParaCertificado(userId, courseId),
  ...
  const colors = {
    primary:       config?.color_primario   ?? '#1E40AF',
    secondary:     config?.color_secundario ?? '#0891B2',
    tertiary:      config?.color_terciario  ?? '#7C3AED',
    background:    config?.color_fondo      ?? '#F8FAFC',
    textBase:      config?.texto_primario   ?? '#0F172A',
    textSecondary: config?.texto_secundario ?? '#475569',
    textTertiary:  config?.color_muted      ?? '#64748B',
    textOnDark:    config?.texto_terciario  ?? '#F5F5F5',
    border:        config?.color_borde      ?? '#E2E8F0',
    input:         config?.color_input      ?? '#FFFFFF',
  };

  const templateBase    = buildCertificateHtml(config?.url_logo ?? '', colors);
  const htmlRenderizado = renderTokens(templateBase, {
    nombre:  datos?.nombre_estudiante ?? 'Estudiante',
    curso:   datos?.nombre_curso      ?? 'Curso',
    fecha:   new Date().toLocaleDateString('es-CO', { year: 'numeric', month: 'long', day: 'numeric' }),
    firma1:  config?.nombre           ?? 'IUSH',
    firma2:  datos?.nombre_docente    ?? '',
  });
