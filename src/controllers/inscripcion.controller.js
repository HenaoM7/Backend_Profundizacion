...
export const create = async (req, res, next) => {
    try {
        ...
        if (!id_curso || !UUID_REGEX.test(id_curso)) return res.status(400).json({
            success: false,
            message: 'El campo id_curso es requerido y debe ser UUID.'
        });
        if (!id_usuario || !UUID_REGEX.test(id_usuario)) return res.status(400).json({
            success: false,
            message: 'El campo id_usuario es requerido y debe ser UUID.'
        });
...
export const remove = async (req, res, next) => {
    try {
        ...
        return res.status(200).json({
            ...
            idInscripcion: deleted.id_inscripcion
        });
    } catch (err) {
        next(err);
    }
...
export const getMisCursosInscritos = async (req, res, next) => {
    try {
        const {id_usuario} = req.params;

        if (!id_usuario || !UUID_REGEX.test(id_usuario)) {
            return res.status(400).json({
                success: false,
                message: 'El id_usuario es requerido y debe ser un UUID válido.'
            });
        }

        // AQUI ESTÁ EL CAMBIO: Se le agrega InscripcionModel. antes de la función
        const cursos = await InscripcionModel.findCursosInscritosPorUsuario(id_usuario);
